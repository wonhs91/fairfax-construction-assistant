...
class MyCrawler:
  def __init__(self, starting_url, base_domain, include_paths=None, store_dir=""):
    self.starting_url = starting_url
    self.base_domain = base_domain
    self.store_dir = store_dir
    self.include_paths = [path.strip('/') for path in include_paths] if include_paths else None
    self.excluding_tags = [
      "header",
      "footer",
      "nav",
  ...
  def clean_filename(self, title: str, url: str, max_length=100) -> str:
  ...
  def save_to_doc(self, markdown, metadata):
    

    filename = f"{self.clean_filename(metadata.get('title', ''), metadata.get('url'))}" + '.json'
  ...
  def scrape_page(self, response):
    ...
    for tag in page_content.find_all(self.excluding_tags):
      tag.decompose()
    ...
    self.save_to_doc(markdown, metadata)
    ...
    for a_tag in page_content.find_all('a', href=True):
      ...
      link = urljoin(self.base_domain, a_tag['href'])
      ...
      if self.include_paths and not self.matches_pattern(link, self.include_paths):
          continue
  ...
  def crawl_url(self, max_size=5, depth=2):
    visited = set([self.starting_url]) 
    url_queue = deque([(self.starting_url, 0)])
    scraped = []
      ...
      try:
        ...
        if response.status_code == 200:
          if curr_url.endswith('.pdf'):
            ...
            continue
          next_links = self.scrape_page(response)
...
crawl_url(starting_url=starting_url, max_size=300, depth=8)
