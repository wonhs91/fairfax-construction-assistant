...
class MyVectorstoreLoader():
  def __init__(self, mode: Literal['local', 'pinecone'] = 'local', collection_name="fairfax-county-construction-code", persist_dir=".\\scripts\\vectorstores\\db"):
    if mode == 'local':
      ...
      self.vectorstore = PineconeVectorStore(index_name=PINECONE_INDEX_NAME, embedding=GoogleGenerativeAIEmbeddings(model="models/text-embedding-004"))
...
# # %%
# import time  
vector_loader = MyVectorstoreLoader(collection_name="fairfax_construction_code", persist_dir=".\\service\\consultation_company\\vectorstores\\db")
